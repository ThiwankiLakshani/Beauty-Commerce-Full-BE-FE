...
import 'package:go_router/go_router.dart'; // <-- IMPORTANT for navigation
...
class _SearchResultsScreenState extends State<SearchResultsScreen> {
  ...
  Future<void> _fetch({bool reset = false}) async {
    ...
    // Strip empty filter values so "clear filters" truly clears
  ...
  void _openFilters() async {
    if (_loadingFilterData) {
      // Prevent opening while metadata is loading
      return;
    }
  ...
  void _onTapProduct(ProductCardItem p) {
    // Use GoRouter route name 'product_detail' (path: /product/:idOrSlug)
    // Since ProductCardItem has no `slug`, just pass the id.
    final idOrSlug = p.id.toString();
    context.pushNamed(
      'product_detail',
